...
import ConsultItem from './CounselingTab/ConsultItem';
...
const AssetCounseling = ({navigation}) => {
    ...
    useEffect(()=>{
        ...
        .then(()=>{
            console.log(tempID);
            console.log(`${url}/requestConsult?userID=${tempID}`);
            // fetch(`${url}/requestConsult?userID=${tempID}`)   //get
    ...
    const loadCounselor = () => {
        setRefresh(true);
        // fetch(`${url}/requestConsult?userID=${userID}`)   //get
        // .then((response)=>response.json())
        // .then((responseJson)=>{
        //     console.log('response data');
        //     console.log(responseJson);
        ...
        return (
            <View style={styles.appSize}>
                <View>
                    ...
                    data={consultData}
                    renderItem={({item}) => <ConsultItem consultNumber={item.consult_number} counselorName={item.counselor_name} consultTitle={item.consult_title} 
                        consultPart={item.consult_part} consultPrice={item.consult_price} counselorCorp={item.counselor_corp} counselorLike={item.counselor_like}
