...
import PensionItems from "../components/PensionItems";
...
function FinancialPensionList(props) {
  const {classes} = props;
  ...
  useEffect(() => {
    axios({
      method:"GET",
      url: '/financialPensionListTotalPage',
    })
    .then((res)=>{
    ...
    .then(()=>{
      axios({
        method:"POST",
        url: `/adminGetFinancialPensionList`,
        data:{
  ...
  function paginateNext(number){
    document.location.href = `/financialPensionList/${number}`;
  }
  ...
  return (
    <div className="NotificationDiv">
      <p className="NotificationTitleText">연금</p>
      ...
      >
        ...
        <option value="연금" selected>연금</option>
      </select>
      </div>
      <div className="NotificationBodyDiv">
        <Paper className={classes.root}>
          <Table className={classes.table}>
            ...
            <TableBody>
              {financialItems.map(c => {return (<PensionItems key={c.product_number} id={c.product_number} category={c.product_category} name={c.product_name} company_name={c.company_name}/>)})}
            </TableBody>
          </Table>
        </Paper>
...
export default withStyles(styles)(FinancialPensionList);