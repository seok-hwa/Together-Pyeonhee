...
import CounselorsFinancial from "../components/CounselorsFinancial";
...
function CounselorListFinancial(props) {
  const {classes} = props;
  ...
  useEffect(() => {
    axios({
      method:"GET",
      url: '/admin/counselorFinancialListTotalPage',
    })
    .then((res)=>{
    ...
    .then(()=>{
      axios({
        method:"POST",
        url: `/admin/getCounselorFinancialList`,
        data:{
  ...
  function paginateNext(number){
    document.location.href = `/counselorListFinancial/${number}`;
  }
  ...
  return (
    <div className="NotificationDiv">
      ...
      >
        <option value="금융상담" selected>금융상담</option>
...
export default withStyles(styles)(CounselorListFinancial);